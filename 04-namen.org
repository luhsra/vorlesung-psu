...
Die gewählte Notation ist die Notation für optionale Parameter bei Latex-Makros.
...
Auf diese Load-Time-Adresse (~0x1000~) können wir den relativen Offset für das Objekt und das Feld aufaddieren, sodass wir das Feld ~obj.p~ an der Speicherstelle ~0x1204~ verorten können.
...
Mit dem Wissen um instanziierbare Namensräume können wir ein C++-Beispiel recht vollständig in unsere Namensraumssprache übersetzen.
...
In den meisten Fällen ist die Beziehung zwischen Aufrufer und Aufgerufenem hierarchisch und der Aufrufer wird solange pausiert bis der Aufgerufene fertig ist.
...
Dabei speichern wir uns einen Zeiger auf den Call-Frame von ~outer()~ an der Stelle, wo ~inner()~ definiert wird, in einem *Closure*-Objekt weg.
...
polymorph.
Bei der Überladung kommt es auf den Kontext des Funktionsaufrufs an. Dort wird ad-hoc entschieden, welche Gestalten ~do()~ haben kann und dann auch einnimmt.

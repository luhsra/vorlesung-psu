...
Ohne Namensräume müsste jeder Name einen Programm-weit eindeutigen Bezeichner haben und es gäbe genau eine lokale Variable mit dem Bezeichner ~i~. Das wäre eher unpraktisch.
...
Die wichtigste Art der Namensauflösung ist die *implizite Namensauflösung*, bei der der Übersetzer automatisch die "nähste" sichtbare Deklaration findet.
...
Dabei bleibt die äußere Deklaration weiterhin bestehen, aber wir können sie mittels der impliziten Namensauflösung nicht mehr referenzieren; sie verliert ihre *Sichtbarkeit*.
...
In unserer Namensraumsprache, können wir mit dem Import Statement (~import <PFAD>~) Namen aus einem explizit benanntem Namensraum importieren.
...
In diesem Unterkapitel werden wir diese Einschränkung aufweichen und es zulassen, das eine Deklaration viele Objekte im Speicher meinen kann.
...
Um die dynamische Namensauflösung wirklich zu verstehen, müssen wir den Begriff der *Binding Time* bzw. Bindungszeitpunkt einführen.
...
Die häufigst genannten ist die Übersetzungszeit (Compile-time binding) und die Laufzeit (run-time binding).
...
Wirklich compile-time binding, wenn wir es ganz genau nehmen, gibt es eigentlich kaum, da die meisten Werkzeugketten für übersetzte Sprachen sich in Übersetzer und Linker aufspalten.
...
Manche Adressen werden erst beim Laden des Programms bekannt, da erst dort entschieden wird an welcher Stelle im Speicher das Programm oder die Bibliothek geladen wird.
...
Diese Relocations haben in etwa die Form:
"Wenn diese Bibliothek an Stelle X geladen wird, dann addiere X auf die Binärcodestelle X[1000] hinzu".
Hierdurch kann der Übersetzer Code erzeugen, der abhängig davon ist, wo eine Bibliothek hin geladen wird.

Noch später findet das *run-time binding* statt, beim dem die Adresse eines Objekts erst zur Laufzeit stattfindet.
...
Obwohl die Adresseberechnung immer erst dann stattfinden kann, wenn eine Bindung zwischen Name und Objekt etabliert ist, können wir doch Teile der Arbeit vorziehen um hinterher Zeit zu sparen.
...
Wir werden nun betrachten, welche Informationen in die Adresseberechnung für den Namen ~*(obj.p)~ eingehen und wann diese zur Verfügung stehen.
Das erste was wissen ist das relative Offset des Feldes ~p~ innerhalb der Struktur ~foo~.
Dieses Offset berechnet der Übersetzer (compile time) , da er entscheidet wie die einzelnen Felder eines Records im Speicher liegen.
...
Allerdings weiß der Linker noch nicht, wo das Program hingeladen wird.
Dieses Wissen haben wir erst zur Load Time, wenn klar ist, wo das BSS Segment im Speicher zu finden ist.
Auf diese Load-Time-Adresse (~0x1000~) können wir den relativen Offsets für das Objekt und das Feld aufaddieren, sodass wir das Felde ~obj.p~ an der Speicherstelle ~0x1204~ verorten können.
Da wir allerdings nicht nur auf ~obj.p~ zugreifen wollen, sondern auf das Objekt auf das dieser Zeiger zeigt, müssen wir zur Laufzeit (run-time binding) noch eine Dereferenzierungsoperation durchführen um die Adresseberechnung abzuschließen.
...
Das bisher vorgestellt Konzept von Namensräumen war sehr statisch und es ging hauptsächlich darum die Deklaration zu einer Referenz zu finden.
Wir haben angenommen, dass hinter jeder Deklaration maximal ein, nach der Bindung genau ein, Laufzeitobjekt/Instanz steht.
...
Denn in dem Moment, wo ich eine Struktur zweimal instantiiere (~objA~, ~objB~), steht die Felddeklaration ~int a~ für zwei Ganzzahlen, jeweils eine in jedem der beiden Objekte.
...
Wenn wir uns zurück erinnern, dann soll jeder Name eindeutig stellvertretend für (maximal) ein Objekt sein.
...
Hinzu kommt, dass bei der Adressberechung von ~objA~, die Teil der vollständigen Namensauflösung ist, noch Laufzeitwissen hinzukommt.
Beide Aspekte, Klonen von Namensräumen und Einbringung von Laufzeitwissen, fassen wir im Begriff der *Instantiierbaren Namensräumen* zusammen.
Dieses Konzept beschreibt, wie wir einen Namensraum, mit all seinen Sichtbarkeits- und Zugriffsregeln, nehmen und unter einem anderen Namen erneut instantiieren.
Somit kann ein Namensraum eine Blaupause für viele andere Namensräume werden.

Das Konzept der instantiierbaren Namensräumen ist ein Schlüsselkonzept der Informatik und manifestiert sich besonders auffällig im objekt-orientierten Programmiersprachen.
...
In unserer Namensraumsprache führen wir, um instantiierbare Namensräume aus zu drücken, das zusätzliche let-Attribut ~let [D] <NAME>~ ein und erlauben, dass wir Instanzen eines Namensraums anlegen können (~<NS>()~).
...
#+begin_src latex
  \begin{frame}<handout:2>[fragile]{Fallbeispiel: Klassen in C++}
    \begin{columns}[t]
      \begin{column}{0.35\textwidth}
        \begin{code}[]
          \begin{CPP}
...
Mit dieser dem Wissen um instantiierbare Namensräume können wir ein C++ Beispiel recht vollständig in unsere Namensraumssprache übersetzen.
Hierbei sehen wir, dass Klassen in C++ die bisher erwähnten Aspekte von Namensräumen (Sichtbarkeit, Import, Instantiierung) miteinander vereint.
...
C++ in seiner Gänze, auch nur den Aspekt der Klassen, können wir nicht allerdings mit unserer Namensraumsprache nicht abbilden.
So gibt es in realen Sprachen noch einige komplexere Randfälle der Sichtbarkeit.
So führt C++ und Java noch das Schlüsselwort ~protected~ ein und es gelten noch leicht andere Regeln wenn Vererbung mit der Verdeckung von Deklarationen zusammenkommt.
Aber ein so tiefer Einstieg in Details einzelner Sprachen ist an dieser Stelle auch nicht da Ziel der Übung.
Sondern ich möchte bei ihnen ein Verständnis für die grundlegenden Namensraumkonzepte schaffen, die Sie dann für jede erlernte Sprache mit um Details ergänzen müssen.
...
Daher gibt es das Schlüsselwort ~static~, dass an bei der Adressberechnung allen Einfluss von dynamischen Informationen abschneidet.
...
Wenn wir die Attribute an den Auflösungspfad schreiben, dann würden bekommen wir ~objO[D]::objI[D]::A[S]~.
...
Die Deklaration lokaler Variablen ist auf den Funktionsrumpf beschränkt und es ist weiterhin nicht möglich auf die lokalen Variablen von andern Funktionen, von außen, zuzugreifen.
Daher können wir sagen, das Funktionen Namensräume sind und die Sichtbarkeit aller enthaltenen Deklarationen ~[I]~ ist.
...
Lösen diese Namen in beiden *Funktionsinstanzen* zum gleichen Objekt auf, oder nicht?
Historisch gesehen, gab es Sprachen die keine Unterstützung für Rekursion geboten haben und eine lokale Variable immer die gleiche Speicherstelle gemeint hat.
...
In kurz: Funktionen sind instantiierbare Namensräume und die lokalen Variablen werden dynamisch zum Call-Frame Objekt aufgelöst.
...
Jetzt ist ~x~ allerdings eine lokale Variable (dynamisch gebunden) und es braucht immer ein Call-Frame Object der Funktion ~outer()~ um eine Adresse für ~x~ zu berechnen.
...
Die Funktion ~inner()~ speichert also dauerhaft den Zustand der umgebenen Funktion zu dem Zeitpunkt an die Closure erzeugt wurde.

Durch das weg sichern des ~outer()~-Call-Frames passieren zwei Dinge:
Zum einen ist die zurückgegebene Funktion nun mit Zustands behaftet, der über mehrere Aufrufen des Rückgabewerts hinweg bestehen bleibt.
Im Beispiel haben wir den Rückgabewert an den Namen ~f~ gebunden und jeder Aufruf von ~f()~ führ dazu, dass die Variable ~x~ um eins erhöht wird.
...
#+begin_src latex
  ...
  \begin{frame}<handout:2,4,5>[fragile]{Pointer, Aliase und Uneindeutigkeit}
    ...
    \ii<4-|handout:4-> \structure{Aliase}: Mehrere Namen (\codeinline{obj}, \codeinline{*ptr}) binden nun das gleiche Objekt.{\\
...
Bei diesem nach gelagerten Schritt dereferenzieren (~(*ptr)~) wir den Zeiger und berechnen die Adresse des Objekts durch einen Blick in den Speicher. Man könnte also sagen, dass ~*ptr~ auch ein Namensauflösungspfad ist, er hängt nur von zusätzlichem Laufzeitwissen ab. Zeiger sind demnach Teil der dynamischen Namensauflösung.

Mit Zeigern bekommen wir nun Situationen bei denen mehrere Namen Stellvertreter für das gleiche Objekt sind, sie sind *Aliase* voneinander.
In unserem Beispiel zeigt der Name ~obj~ und der Pfad ~*ptr~ auf das gleiche Objekt, obwohl die Deklarationen beider Namen nichts miteinander zu tun haben.
Die beiden Namen wurden erst zur Laufzeit an das gleiche Objekt gebunden.
...
Das kann doch irgendwien nicht sein, wie sollten wir denn die Adresse ~&fun~ berechnen?
...
Überladung ist ein Teil des Konzepts *Polymorphismus*, dass wir bereits im Typenkapitel kennen gelernt haben.
...
Bei der Überladung von Funktionen haben wir das Funktionssymbol (~do()~), dass sich sowohl wie ein ~do(Point2D)~ als auch wie ein ~do(Point3D)~ verhalten kann; es hat also mehrerlei Gestalt.
...
polymorph.
Bei der Überladung kommt es auf den Kontext der Funktionsaufrufes an, dort wird ad-hoc Entschieden welche Gestalten ~do()~ haben kann und dann auch einnimmt.
...
Um unser Konzept von Namen, bei dem jeder Name stellvertretend für genau ein Objekt steht, aufrecht zu erhalten, müssen wir unsere Namen erweitern, damit Sie, trotz Überladung, eindeutig bleiben.
...
Die meisten Sprachen schauen sich bei der Überladung nur die Paramtertypen, nicht aber den Rückgabetypen an[fn::Die Sprache Ada ist ein Gegenbeispiel.
Dort findet Überladung auch anhand des Rückggabetypen statt.].
Daher wollen wir uns auf diese gängigste Art der Überladung beschränken.

Bei der Überladung wir die Signatur also zum Teil des Namens.
...
Dazu wollen wir uns anschauen, wie überladene Funktionen für ein monomorphes und statisches Typsystem augelöst werden[fn::Zur Erinnerung: Monomorph=Typen sind gleich oder inkompatibel zueinander. Statisch: Variablen haben annotierte Typen.]
...
Die kann mittels CNSR oder über expliziter Namenspfade passieren.
...
An dieser Stelle haben wir eine Interaktion zwischen Typssystem und Namensauflösung, weswegen Überladung auch eine *typabhängige Namensauflösung* ist.

Aus dem vervollständigten Overload Set streichen wir zuerst alle Deklarationen bei der die Anzahl der Parameter nicht übereinstimmt (~f(int, int)~).
...
Dort gibt es das sogenannte *Name Mangling* bei denen die Parametertypen in den Assemblernamen der Funktion codiert wird.
...
Mit diesem Wissen um Überladung wollen wir uns nun einem Thema nähern, dass für viele eine Art schwarze Magie zu sein scheint: *Operatorüberladung*. Dabei ist die Überladung von Operatoren eigentlich auch nicht anders als die Überladungen von Funktionen. Wir müssen uns nur folgenden Satz auf der Zunge zergehen lassen: *Operatoren sind komisch benannte Funktionen*.
...
Klarer wir das ganze, wenn wir einen Ausdruck mit einem Operator nicht als Infix[fn::Der Operator steht zwischen seinen Argumenten], sondern in Prefixschreibweise notieren:
~+(A, B)~.
...
So haben die Überladungen des Plus-Operator in Python nicht nur unterschiedliche Datentypen (~float~ vs.
~int~), sondern es steht manchmal sogar eine ganz andere Semantik dahiner, wie dies bei ~+(string, string)~ der Fall ist.
...
In C++ wird dazu das Prefix ~operator~ vor den Operator geschrieben.
...
Dazu muss allerdings Lexer und Parser, während diese gerade laufen, über die neu definierten Operatoren informiert werden.
...
Im letzten Unterkapitel haben wir die (vorläufige) Annahme getroffen eine Sprache mit monomorphem Typsystem zu haben. Allerdings haben wir in der letzten Vorlesung schon gesagt, dass solche Typssystem wenig flexibel sind. Daher wollen wir uns nun ansehen, wie Überladung mit einem Inklusionspolymorphismus interagiert und welche Folgen sich daraus ergeben. Wir picken uns für diese Diskussionganz bewusst den *Subtyp*-Polymorpismus heraus, weil dieser allgegenwärtig ist (Vererbung).

Durch die Einführung von Subtyping erzeugen wir eine gewisse Uneindeutigkeit mit denen die Sprache umgehen muss um weiterhin die Regel "Ein Name, ein Objekt" aufrecht zu erhalten. Dabei bekommen wir an zwei Stellen Uneindeutigkeit: An der Deklarationsstelle und an der Aufrufstelle.
...
Dazu rechnen wir für eine bestimme Aufrufstelle die *Spezifität* für jede Deklaration des Overload Sets.
...
In dem Moment, wo wir das Typssystem nach der Signatur der Aufrufstelle beten, fragen wir nicht nach der Signatur der statischen Typen, sondern nach der Signatur der dynamischen Typen.
Es werden also nicht die Typen der Variablen verwendet, sondern der Typ des übergebenen Objekts zählt. Eigentlich ganz einfach, könnte man Browser und Laptop zuklappen und endlich schlafen gehen. Leider ist es nicht ganz so leicht.
...
Wir müssten also ganz oft mit der Auflösung der Überladung bis zum letzten Moment, bis zur Nanosekunde des Aufrufstelle warten, die dynamischen Typen extrahieren und die ganze Berechnung der Spezifität durchführen.
...
Das ist schade, weil es uns solch schreckliche ~isinstance()~-Kasakaden wie im Beispiel ersparen würde.
...
Methoden werden im Kontext eines dynamisch instantiierbaren Namensraums (zum Beispiel einer Klasse) deklariert.
...
Virtuelle Methoden lassen sich relativ effizient implementieren, indem man eine virtuelle Funktionstablle (vtable) in jedes Objekt schreibt. Die Idee dabei ist die Folgende:
...
5. Wir schreiben einen auf die passende Implementierung in die passende Funktionstabelle.
...
An der Aufrufstelle erstellen wir dann nur noch den statischen Anteil der Signatur, führen die Überladungsauflösung durch um auf die Slotnummer zu kommen.
Und erzeugen eine Call Instruktion, die dynamisch, über den ~this~- und den ~vtable~-Zeiger auf die richtige Implementierung kommt.
...
- ~{[<ATTR>] ...}~: Ein Namensraum mit Standardattribute, die er an seine enthaltenen Deklarationen vererbt, die dort aber überschrieben werden können.
...
- ~[E]~: Externe Sichtbarkeit. Das Gegeteil von ~[I]~.
- ~[D]~: Der so deklarierte Name benötigt Laufzeitwissen um aufgelöst zu werden.
- ~[S]~: Alles Laufzeitwissen wird bei der Auflösung dieses Namens ignoriert bzw. ist nicht nötig. Gegeteil von ~[D]~.

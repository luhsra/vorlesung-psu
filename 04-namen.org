...
Und das kann ja irgendwie nicht sein. Wir müssen also unseren Formalismus und die Mini-Sprache erweitern um auch dieses Sprachkonstrukt abzudecken.
...
In unserer Mini-Sprache wäre es ungefähr:
...
In unserer Mini-Sprache führen wir, um instantiierbare Namensräume aus zu drücken, das zusätzliche let-Attribut ~let [D] <NAME>~ ein und erlauben, dass wir Instanzen eines Namensraums anlegen können (~<NS>()~).
...
C++ in seiner Gänze, auch nur den Aspekt der Klassen, können wir nicht allerdings mit unserer Mini-Sprache nicht abbilden.
...
Um dieses Verhalten in unserer Mini-Sprache abzubilden führen wir das let-Attribut ~[S]~ ein, das alle ~[D]~ Attribute im Auflösungspfad ungültig macht.

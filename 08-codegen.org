...
#+begin_src latex
  ...
  \begin{frame}[fragile]{Bottom-Up Codeerzeugung für die Addition}
    ...
    \ii Explizite Traversierung bei der Codeerzeugung{
      ...
      \ii Generierungs-Funktionn verwenden Referenz auf aktuelle Funktion/Block
...
Allerdings verwenden wir nicht nur ein Set an ~visit_*()~-Methoden, sondern gleich drei Verschiedene, je nachdem ob der Unterbaum zu Code für einen L-Wert (~lvalue_*()~), für einen R-Wert (~rvalue_()~), oder ganz ohne Rückggabewert (~visit_*()~) erzeugt werden soll.
...
Register-Maschnen angedeutet, berechnet der Code für einen Ausdruck einen Wert und speichert ihn in einer Variable.
...
In diesem winzigen Ausschnitt, der R-Wert Code für ein konstantes Literal erzeugt[fn::~lvalue_Literal()~ kann es nicht geben. Überlegen Sie sich warum!], wird eine Variable optional Top-Down übergeben. Hat der Aufrufende keine Variable mitgegeben, so erzeugen eine an dieser Stelle. In allen Fällen propagieren wir die Zielvariable als Rückgabewert.
...
Genau diese Stellen in der Codeerzeugung, bei der wir uns auf eine Reihenfolge der Kindsknoten festlegen, sind es die die *Linearsierung des ASTs* durchführen.
...
Zuerst erzeugen wir den R-Wert Code für jeden Unterbaum und speichern das Ergebnis in jeweils einer Variable, bevor wir diese im Knoten-spezifischen Code verarbeiten, der wiederum eine Ergebnisvariable beschreibt.
...
Dieses allgemeine Vorgehen (Wert von rechts, Zeiger von links, Store) funktioniert in jeder Situation, egal wie komplex der Ausdruck wir auf der linken Seite ist (z.B.
~*(foo.bar()) := ...~).
...
Geht es nun daran Code für diesen Block-Knoten zu generieren, sieht die Erzeugungsmethode denkbar einfach aus, denn sie besucht, in der gegebenen Reihenfolge, alle Statements des Blocks mit der ~visit()~-Funktion (unsere Statements haben keinen Rückggabewert).
...
Egal wie, nach dem Besuchen des Then-Zweiges, fügen wir ein ~Goto .BB3~ ein um den Kontrollfluss wieder zusammen zu führen.
...
Am Ende der Selektion stellen wir die Sequenzierungs-Invariante wieder her, indem wir den ~current_block~ auf BB3 setzen, welcher im CFG von BB1 dominiert wird.
...
Wiederum könnte der Kontrollfluss innerhalb des Schleifenkörpers gespalten werden, jedoch ist dieser am Ende wieder eindeutig und wir können einen *Rücksprung zum Schleifenkopf* einfügen um die nächste Runde der Iteration ein zu läuten.
...
Wenn wir mit bloßem Augen auf solchen IR-Code schauen, könnten wir auf die Idee kommen zu sagen, dass wir doch direkt besseren IR-Code erzeugen könnten. Jedoch spricht ein wichtiges Argument gegen eine bessere Coderzeugung: Komplexität.

Die Aufgabe der Codeerzeugung ist es den AST für eine lineare Maschine zu sequenzieren.
Alles was wir an zusätzlicher Logik in diesem Übersetzer-Schritt einbauen um besseren IR-Code zu generieren wird die Codeerzeugung verkomplizieren.
...
In diesem Stück Beispielcode ist, bei nähererer Betrachtung, ganz klar, dass ~foo()~ immer das Ergebnis 3 liefern wird.
Eine Codeerzeugung zu erstellen, die dies nebenbei heraus findet ist um Größenordnungen schwieriger, als was ich Ihnen in dieser Vorlesung gezeigt habe.
...
Dort können wir, um den Start der Ausführung nicht übermäßig zu verzögern, nicht zuerst aufwändige Optimierung vorschalten.
Daher treffen Codeerzeugungs-Routinen, die eine interpretierte Ausführung zum Ziele haben, andere Abwägungen und führen Coderzeugung und Optimierung deutlich vermischter aus.
...
Daher habe ich mich entschieden ein einzelnes Thema heraus zu greifen und an diesem beispielhaft zu demonstrieren, wie wir bei der Codeerzeugung vorgehen.
Das Thema dem wir uns zuwenden wollen sind *Records*{{{see(03-composite-types,Zusammengesetzte Typen)}}}, da wir an ihnen einer Vielzahl der Abbildungsproblemen begegnen, die auch bei anderen Sprachkonstrukten auftreten.

Di *semantische Lücke* der wir bei Record-Datentypen begegnen, lässt sich wie folgt beschreiben:
...
Dadurch das Records Verbünde aus mehreren Feldern sind, können Record-Objekte mehr als ein Maschinenwort umfassen.

Das Problem ist, das bei unserer IR-Maschine alle Variablen genau ein Maschinenwort (32-Bit) breit sind.
Ein Variable kann also nicht jedes beliebige Record-Objekt beinhalten.
Natürlich könnten wir hergehen und unseren IR-Code erweitern um eine möglichst einfache Codeerzeugung zu bekommen, jedoch würde wir damit die Übersetzungsleistung einfach weiter nach hinten ins Backend schieben.
...
Anstatt dessen werden wir das Sprachkonstrukt "Record" auf den vorgestellten IR-Code abbilden.
...
Das Datenlayout ist abhängig von Record-Typen und kann von sprachspezifischen und architektonischen Besonderheiten beeinflusst. So gibt der C-Sprachstandard vor, dass Felder beim Datenlayout nicht umgeordnet werden dürfen.

Als erstes berechnet der Übersetzer wie viel Speicher jedes einzelne Feld des Records benötigen würde, wenn es alleine alloziert wird.
...
Außerdem werden die Informationen über das Datenlayout auch in den Informationen für den Debugger vermerkt, sodass dieser ein solches Objekt aus dem Speicher dekodieren kann und die semantische Zuordnung zwischen Feld und Speicherzelle rückgängig machen.

Wollen wir dynamische Typ-Informationen für unserer Objekte haben, so müssen wir ein zusätzliches Typ-Tag Feld im Datenlayout vorsehen.
...
Für unsere Beispiel werden wir ein *Wertemodell für Variablen* anstreben, da wir die Records in eine Sprache mit Referenzen einbauen wollen.
Zur Erinnerung:
...
Diese drei Stellen werden wir uns im Detail ansehen ansehen.
...
Erst der Verwender des R-/L-Wertes entscheidet dann ob er aus dem Speicherbereich ließt oder schreibt.
...
Mit dem Wissen, dass bei Objekten, die größer sind als eine einzelnes Maschinenwort, der L- und der R-Wert zusammenfallen, können wir nun auch in der Zuweisungsoperation einen weiteren Sonderfall (neben dem für Zuweisung zu Variablen) einbauen.
...
Die Anzahl der Bytes, die kopiert werden muss ergibt sich aus der Berechnung des Datenlayouts.
...
Dazu legen wir eine Aufrufkonvention fest: Zwar bekommt der Aufgerufene einen Zeiger, legt sich aber eine selbständig eine Kopie in seinem Call-Frame an, dann im folgendem anstatt des Parameters verwendet wird.
...
Ein ähnliches Problem haben wir bei der Rückgabe des Objekts. Wiederum wollen wir, auf Sprachebene, das Objekt "by-Value" zurückgeben, können aber nur Zeiger zwischen Caller und Callee austaschen. Da die Lebenszeit des Rückgabewerts länger sein muss als die Laufzeit der Callee-Funktionsinstanz, müssen wir auch Speicher aus dem Call-Frame des Aufrufendem verwenden um das Objekt zu transportieren.
...
Wir haben gesehen, dass die Codeerzeugung der Zuweisung für die linke Seite einen Zeiger als L-Wert berechnet und den zugewiesenen Wert als R-Wert. Also können wir die Frage nach dem Feldzugriff umformulieren und Fragen: Was ist der L-Wert eines Feldzugriffes und was ist sein R-Wert?

Für den L-Wert müssen wir die Adresse des Feldes berechnen. Diese ergibt sich daraus, dass wir auf die Basisadresse des Objekts den Offset des (statisch benannten) Feldes aus dem Datenlayout addieren. Auf den Folien sehen wir wie dies alles ineinander bei ~lvalue_FieldAccess()~[fn::FieldAccess ist der AST Knotentyp für den Punkt-Operator: "lhs.name"] ineinander greift: 
Um die Basisaddresse der ~lhs~ zu berechnen brauchen wir den L-Wert des Objekts (also die Adresse des Objekts). Aus dem Feldnamen ~name~ leiten wir, zusammen mit dem Datenlayout das konstante Offset ab und addieren Basisaddresse und konstantem Offset zum L-Wert des Feldes.
...
Zu diesem Zwecke kommt ein Übersetzer nicht nur als ein Binärprogramm, welches aus dem Quellcode ein Assemblerprogramm macht, sondern es gibt sogenannte *Run-Time Support Libraries*.

...
#+begin_src latex
  ...
  \begin{frame}{Komplexere Objekte: Records}
    \begin{btBlock}{Intentionale Unvollständigkeit der Vorlesung}\small
      Der \advantage{Grundstock der Codeerzeugung} ist gelegt und kann beliebig erweitern werden. Um ihnen einen Einblick zu geben, ohne Sie mit endlosen Erzeugungsregeln zu langweilen, werfe ich nur ein \structure{Schlaglicht auf zwei Themen}.
    \end{btBlock}

    \bi
    \ii Records haben mehrere benannte und typisierte Felder {
      \bi
      \ii Ein Record-Objekt kann größer sein als ein einzelnes Maschinenwort
      \ii \ALERT{Aber}: Die IR-Maschine kann nur 32-Bit Zahlen speichern und verarbeiten
      \ei
    }\medskip
    \ii Wir müssen eine Übersetzung des Sprachkonstrukts vornehmen:{
        \bi

...
Mit der Definition auf den Folien verknüpfen wir die Lebenszeit eines Objekts mit seiner Nützlichkeit.
Wenn wir seinen Nutzen, der durch die enthaltenen Informationen oder seine pure Existenz[fn::Stellen Sie sich ein Objekt vor, das solange es existiert ein Relais geschlossen hält.
Dieses Objekt ist sogar dann nützlich, wenn wir nicht mehr darauf zugreifen. Aber weil dieser Fall eher Esoterisch ist, ignorieren wir ihn ab jetzt.], weiterhin benötigen.
Wie wir schon sehen, genauso wie Besitz, ist die genaue Feststellung der Lebenszeit schwierig, da wir im vor hinein nicht wissen, ob später noch einmal ein Zugriff auf das Objekt stattfinden wird.
...
Wird ein Referenzobjekts gelöscht, weil zum Beispiel die lokale Variable in der es lebt ungültig wird, so ruft der Destruktor ~release()~ auf. 

...
#+BEGIN_SRC latex
  ...
  \bi
  \ii Vorlesung: 13 Termine {
    \bi
    \ii Dienstag 9:00-11:00 Uhr, 201, Conticampus
  ...
  \ii Wie funktioniert der Übungsbetrieb? {
    ...
    \ii Tafelübung: Dienstag 16:00-18:00 Uhr
  ...
  \ii Bonuspunkte auf die Klausur (wie bei GBS) {
    ...
    \ii Testatklausur am \alert{23. Juni 2020}, maximal +10\% der Klausurpunkte
...
Hierbei sind Programmiersprachen ein ganz essentieller Teil bei einem der grundlegendsten Problemen der Informatik:
...
Diesen Zwiespalt nennen wir auch **semantische Lücke**, da zwischen einer mathematischen Formulierung und dem existierenden Silizium eine große Lücke klappt.
Dank Alan Turing und Alonzo Church, wissen wir das sehr einfache Turingmaschinen bereits alles berechnen können, was berechnet werden kann; aber bequem geht es dabei keinesfalls zu.
...
Dabei kann die Abbildung auf die darunterliegende Maschine entweder *statisch* durch eine Übersetzung oder *dynamisch* durch Interpretation erfolgen.[fn::Die Unterscheidung zwischen statisch (vor der Laufzeit) und dynamisch (zur Laufzeit) wird uns durch das ganze Semester hindurch begleiten. Es das erste Kernkonzept von Programmiersprachen dem wir begegnen; man kann es  als die Frage "Was passiert in einer Sprache statisch und was passiert dynamisch?" formulieren.]

Im konkreten Falle eines Arbeitsplatzrechers, ordnet sich die Hochsprachenebene direkt über der Ebene der Assemblersprache ein.
Das, in den Folien gezeigte, Bild kennen sie bereits aus der Veranstaltung "[[https://www.sra.uni-hannover.de/p/lehre-V_GBS][Grundlagen der Betriebssysteme]]", in der sie sich mit der Maschinenprogramm und Befehlssatzebene auseinander gesetzt hat.
Zur Erinnerung:
das Betriebssystem nimmt eine Teilinterpretation des Anwenderprogamms vor, indem es jeden Systemaufruf (~int 0x80~, ~sysenter~) aus dem Maschinenprogramm interpretiert, aber alle anderen Befehle (~mov~, ~add~) direkt auf der Befehlssatzebene ausführt.
In PSÜ, werden wir uns nun nicht mit (Teil)Interpretation auseinander setzen, sondern mit der Übersetzung von Hochsprachenprogrammen in Assemblersprachenprogrammen.
Dabei werfen wir auch noch kurzen Blick auf den nachgelagerten Schritt der Linkens (FIXME), der neben dem Assemblieren, die letzten Schritte zum ausführbaren Binärprogram fehlt.
...
Unsere virtuellen Maschinen haben Schnittstellen die unterschiedlich sind von den Maschinen auf denen sie implementiert werden.

Grundlegend besteht eine virtuelle Maschine aus zwei Komponenten:
dem *Speicher* und den angebotenen *Befehlen*.
Dabei können Befehle Daten aus dem Speicher lesen, verarbeiten sie, und schreiben die Ergebnisse wieder zurück. Die Gesamtheit des Speichers zu einem Zeitpunkt ist der *Zustand* der virtuellen Maschine.
...
#+begin_src python
  def prozessor(memory, program, maximal_steps=10):
      """Prozessor fuer eine virtuelle Maschine mit 2 Befehlen: add und print"""
...
Ein Programm unserer Beispielsprache ist eine Liste von Befehlen, die jeweils aus Operation und Operand bestehen; dabei muss die Operation eine der Zeichenketten ~"add"~ oder ~"print"~ sein.
Der Operand ist die Adresse einer Speicherzelle, derer wir 5 haben, kodiert als Ganzzahl.

Mit Hilfe dieser Sprachdefinition, können sie nun noch einmal an die Aufgaben 3 und 4 herangehen und werden sehen, dass das einfügen eines invaliden Befehls zwar Vordergründig zu einem ~RuntimeError~ geführt hat, aber der Prozessor eigentlich richtig gehandelt hat.
Denn das übergebene Programm was leider kein Program, welches gegen unser Maschinenmodell programmiert wurde, es hatte nur Ähnlichkeit mit einem validen Programm. Ähnlich Verhält es sich mit der zu großen Speicheradresse (4); auch dort war das übergebene Programm nicht von unserer Sprachdefinition abgedeckt.
...
FIXME: Die Folien hier könnten auch in die Optimierungsvorlesung passen.
...
Zum ersten ist es natürlich notwendig, das ein Übersetzer für jedes valide Quellprogramm, was er durch seine Validierungsfunktion sicherstellt, ein valides Zielprogramm erzeugt.
Diese notwendige Bedingung ist allerdings ganz sicher nicht hinreichend, da ein Übersetzer, der immer dasselbe Programm ausgibt, egal mit was wir ihn füttern, ganz sicher nicht nützlich ist.

Daher müssen wir noch einmal zu unserem Maschinenmodell zurückkehren und schauen was wir auf vom Übersetzer erwarten.
Wir möchten, dass ein Übersetzer ein Zielprogramm erzeugt, dass sich so verhält wie das Quellprogramm es getan hätte.
...
Meistens stellt das Maschinenmodell aber noch weitere Regel für eine korrekte Übersetzung auf.

Bei dieser erweiterten Definition von korrekter Übersetzung, erwarten wir nicht nur das Zielprogramm das gleiche Ergebnis berechnet un uns mitteilt, sondern auch dass es dies auf die gleiche Weise tut.
Das Zielprogramm soll, im Grunde, die gleichen Schritte durchführen wie das Quellprogramm, nur auf einer anderen virtuellen Maschine. Um dies formaler zu fassen, führen wir auf den Folien den Begriff der *beobachtbaren Zustand* ein.
...
Im Grunde legt das Maschinenmodell fest, welcher Teil des Zustandes wichtig sind und als beobachtbar gelten.
Zum Beispiel, legt die Programmiersprache C fest, dass die Werte von globalen Variablen beobachtbar sind.
Der Hintergrund dieser Entscheidung ist, das andere Threads die geschriebene Werte von globalen Variablen sehen, wenn die Zuweisung ausgeführt wurde und nicht erst am Ende der Funktion, oder gar noch später.

Eine andere wichtige Erkenntnis aus der Defintion der korrekter Übersetzungen ist das *nur* der beobachtbare Zustand erhalten werden muss.
Alles was die Sprache als nicht beobachtbar deklariert, kann vom Übersetzer beliebig verändert werden.
Zum Beispiel, entfernen Übersetzer für C lokale Variablen aus Funktionen, die niemals verwendet werden.
Dies ist laut C Standard eine völlig valide Optimierung, da die Werte von lokalen Variablen nicht beobachtbar sind.
Umso mehr ein Machinenmodell als nicht beobachtbar deklariert, umso leichter fällt es einem optimierenden Übersetzer ein effizientes Zielprogramm zu erzeugen.
...
Wir haben bereits gesehen, dass Übersetzer Programme von einer Sprache in eine andere Sprache, unter Beibehaltung der beobachtbaren Zustände, transformieren.
...
Allerdings ist es sehr aufwendig einen Übersetzer direkt in M zu schreiben, denn M hat tradtionellerweise weniger nützliche Abstraktionen als L (Ebenenmodell).
Einen Übersetzer in Assembler zu schreiben macht einfach keine Freude.
Ausserdem wäre dann unser mühsam erstellter Übersetzer spezifisch für unsere Zielplattform und wir müssten mit jeder neuen Prozessorarchitektur einen neuen Übersetzer erstellen.
Daher wollen wir lieber den Übersetzer in der Sprache L schreiben.

Dies hat auch den Vorteil, dass wir, als neue Sprachentwickler, gleich die Vor- und Nachteile unser neuen Sprache kennenlernen und vielleicht noch Anpassungen an der Sprache vornehmen.
...
Weil man sich in diesem Prozess quasi selbst an den Haaren aus dem Sumpf zieht, heißt dieser Prozess *Bootstrapping*.
Außerdem gehört es quasi zum guten Ton eines Sprachentwicklers, wenn sein Übersetzer in der eingenen Sprache geschrieben ist und sich selbst übersetzen kann[fn::[[https://en.wikipedia.org/wiki/Self-hosting_(compilers)][Wikipedia: Self-Hosting Compiler]]].
...
Viel schwerwiegender ist allerdings das ein Konsistenzproblem.
Denn woher wissen wir, dass Interpreter und Übersetzer wirklich die gleiche Sprache implementieren oder ob sich doch in einem von beidem ein Fehler eingeschlichen hat.

Die andere Möglichkeit, verwendet eine Interativen Prozess der Selbstübersetzung einer vereinfachten Variante des Übersetzers.
Nachdem man der Übersetzer (L->M) in L geschrieben hat, definiert man die Untermengen L' und M', die gerade noch so mächtig sind um einen vollständigen Übersetzer zu schreiben.
Dies kann passieren, indem man exotische Sprachfeatures in L' weglässt bzw. die komplexeren Maschinenbefehle die M eigentlich bietet ignoriert.

Mit diesen Untermengen, leitet man aus dem eigentlichen Übersetzer einen vereinfachten Übersetzer her, der in L' implementiert ist und nur ineffizienten Code für die M' erzeugt.
...
Und obwohl unser vereinfacher Übersetzer eigentlich L übersetzen kann, können wir bei diesem manuellen Schritt sogar noch mehr Aspekte weg lassen, sodass am Ende nur ein Übersetzer für ~L'->M'~ entsteht.
...
Damit bekommen wir eine volle Version des vereinfachten Übersetzers, den nun wirklich den gesamten Umfang von L versteht und verarbeiten kann.
Als nächstes Stecken wir den orginalen Übersetzer in diesen vereinfachten Übersetzer und bekommen eine langsame Variante des orginalen Übersetzers, der auf M' läuft, aber schon Code für M erzeugt.
Eine weitere Interation der Selbstübersetzung, liefert nun endlich das gewünschte Ergebnis: einen schnellen Übersetzer, der unsere Sprache L verarbeitet und Programme für M erzeugt.
Wiederholt man der Vorgang ein weiteres Mal und vergleicht die Binärdateien der beiden letzten Übersetzer, so kann man noch weiteres Vertrauen in die Übersetzungsleistung gewinnen[fn::Ein interessantes Beispiel eines absichtlichen Fehlers beim Bootstrapping ist das klassische Paper von Ken Thompson, einem der Erfinder von C, [[https://dl.acm.org/citation.cfm?id=358210][Reflections on trusting trust]].
...
Ein ähnliches Probelm wie beim Bootstrappen tritt bei der Portierung eines Übersetzers auf eine neue Hardwarearchitektur auf.
...
Modernere Übersetzer sind Multi-Targetfähig und sind von Anfang an darauf vorbereitet Code für  unterschiedliche Architekturen zu erzeugen (siehe LLVM Graphik weiter oben).

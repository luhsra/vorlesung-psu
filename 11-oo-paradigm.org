...
#+begin_src latex
  ...
  \begin{frame}{In the Beginning,\ldots}
    ...
    \ii   \ldots all data was unstructured and all operations were floating.{
      \bi
      \ii[+=] \structure{Records}: Daten werden zu Datenstrukturen zusammengefasst.
      \ii[+=]\structure{Sequenzierung}: Befehle nacheinander ausführen. Ergebnisse weitergegeben.
      \ei
    }\medskip
    \ii Beispiel: Reverse Polish Notation (wie in PostFix) {
      \bi

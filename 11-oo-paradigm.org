...
#+begin_src latex
  ...
  \begin{frame}{Probleme imperativen Programmierens}
    ...
    \ii Besonders problematisch bei großen Softwareprojekten {
      \bi
  ...
  \begin{frame}[fragile]{Die Methode: Die zentrale Erweiterung}
    \bi
    \ii Methoden haben einen \structure{Ausführungskontext} mit dem Objekt{
      \bi
      \ii Objekt wird implizites Argument der Methode.
      \ii Felder des Objekts sind im Methodenrumpf sichtbar.
      \ii Methode ist eine \structure{Closure}! (Siehe \enquote{03 - Namen})
      \ei
    }
    \ei
    ...
    \bii
    \ii<3-> \codeinline{obj.increment} $\Rightarrow$ \codeinline{<bound method C.increment of <C instance at 0x7f...>>}{
      \bi
      \ii \codeinline{obj.increment.__func__} $\Rightarrow$ \codeinline{<function increment>}
      \ii \codeinline{obj.increment.__self__} $\Rightarrow$ \codeinline{<C instance at 0x7f...>}
      \ei
    }\medskip
    \ii<4-> \codeinline{C.increment} $\Rightarrow$ \codeinline{<unbound method C.increment>} {
      \bi
      \ii \codeinline{C.increment.__func__} $\Rightarrow$ \codeinline{<function increment>}
      \ii \codeinline{C.increment.__get__(obj)} $\Rightarrow$ \codeinline{<bound method ...>}
      \ei
    }
    \eii
  \end{frame}
...
\dividerframe{Prototypen}
  \begin{frame}{Objekt-Orientierung ohne Klassen}
    ...
    \ii Javascript kennt \ALERT{keine Klassen} und ist \ADVANTAGE{dennoch} Objekt-Orientiert{
      ...
      \ii Javascript-Objekt haben Felder (\C) gebundene Methoden (\C)
  ...
  \begin{frame}[fragile]{JavaScript: Die Prototypen Kette}
    ...
    \ii<2-> Die \texttt{\_\_proto\_\_}-Property: Fallback für den Property-Zugriff{
      \bi
      \ii Situation: Ein Objekt hat die geforderte Property nicht
  ...
  \begin{frame}[fragile]{JavaScript: Property-Lookup und \texttt{this}}
    \bi
    \ii Prototypen-Kette implementiert dynamischen und hierarchischen Lookup{%
      ...
      \ii Speichereffizient, aber mit hohen Lookup-Kosten verbunden.
      \ei
    }\medskip
    \ii<2-> JavaScript bindet den Namen \structure{\texttt{this}} an das aktuelle Objekt{
      \bi
      \ii Properties können auf Funktionen verweisen.
      \ii Property als Funktion aufrufen $\Rightarrow$ Objekt wird an \texttt{this} gebunden
      \ei
    }
    \ei
    \begin{columns}<2->
      ...
      \begin{column}{0.49\textwidth}
        \bii
        \ii Aufruf der Funktion als Property\\
  ...
  \begin{frame}[t,fragile]{JavaScript: Funktionen als Konstruktoren}
    ...
    \ii \textbf{Alle} Funktionen können als Konstruktoren verwendet werden{
      ...
      \ii Jede Funktion hat eine \structure{\texttt{.prototype}}-Property, die als \texttt{\_\_proto\_\_} gesetzt wird.
      \ei
    }
    \ei
  ...
  \begin{frame}{Prototypen-basierte Objektorientierung}
    ...
    \ii Veränderung eines Prototypen ändert alle abgeleiteten Objekte {
      \bi
      \ii JavaScript: Eingebaute Klassen können erweitert werden (discouraged)

...
#+begin_src latex
  ...
  \begin{frame}[t]{Der Kontrollflussgraph}
    ...
    \end{btBlock}\only<2>{\textbf{Enschuldigung} \btSetTab für die Definitionen, aber der CFG ist \\
      \btUseTab die zweite \ALERT{zentrale Datenstruktur} des Übersetzerbaus.}\par
    \bi
    \ii<3-> Fakten für Basisblöcke, die aus den Definitionen folgen{
      \bi
    ...
    \ii<4-> Fakten über den CFG, die aus den Definitionen folgen{
      \bi

...
\begin{frame}[fragile]
  \begin{tikzpicture}[remember picture]
    ...
    \node[anchor=north,below=1 of relocs,
      ...
      e8 \tikznode[text width=1.4cm,draw,fill=srared!20,name=fib-call]{00 00 00 00}\\
      .. .. .. .. c3
    };
    ...
    \draw[->] (reloc2.west) -- ++(west:1.2ex) |- (sym2-fib.west);
    \draw[->] (reloc2.east) -- ++(east:1ex) |- (fib-call.east);

    \node[rotate=90,anchor=south,yshift=1ex] at (section-text-label) (section2-label)
      {\scriptsize Sections};
    
    \node[draw,fit=(section2-label) (section2-text) (relocs2),
         label={[anchor=south west]south west:foo.o}
         ](object-foo) {};
    ...
    \node[below=0.2 of main.south west,align=left,anchor=north west,xshift=-2.8cm]{
      \begin{minipage}{7.2cm}
        \bii
        \ii[--] Sektionen einsammeln und kombiniern
        \ii[--] Relokationen (wenn möglich) auflößen

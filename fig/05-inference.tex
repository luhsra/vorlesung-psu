...
\begin{frame}[fragile]
  \begin{tikzpicture}[
    ...
    \begin{visible}<2>
      \node[align=center,left=0.4 of call.two split west,anchor=east](@){Typregel\\für\\CallExpr};
      \draw[ultra thin] (@) -- (call.three west);
    \end{visible}

    \node[below=0.5cm of p1,align=center,draw,visible on=<3->] {
      \textbf{Ersetzung der Typvariablen}\\
      \strut\only<4->{$\alpha\rightarrow[\gamma]$}\\
      \strut\only<5->{$\beta\rightarrow[\gamma]$}\\
      \strut\only<6->{$\gamma\rightarrow \GAMMA$}
    };
  \end{tikzpicture}
\end{frame}
\end{document}

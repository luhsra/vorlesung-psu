...
\tikzset{
	...
	nothing/.style={},
	layers/.style={code={%
		\node[box, label=\L0] (dle)                 {Logikebene};
		\node[box, label=\L1, above of=dle] (mae)   {Mikroarchitekturebene};
		\node[box, label=\L2, above of=mae] (bse)   {Befehlssatzebene};
		\node[box, label=\L3, above of=bse] (mpe)    {Maschinenprogrammebene};
		\node[box, label=\L4, above of=mpe] (ase)    {Assemblersprachenebene};
		\node[box, label=\L5, onslide=<2>{fill=luhblue,text=white},above of=ase] (pose)   {Hochsprachenebene};

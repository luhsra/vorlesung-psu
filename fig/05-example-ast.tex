...
\begin{frame}<1-6>[fragile]
  \begin{tikzpicture}[
    ...
    \begin{visible}<4->
      \node[type at node=plus] (plus-type)   {(int,int)$\rightarrow$int};
      \begin{visible}<4>
        \draw[ultra thin] (plus-type.north) -- ++(20:1em) node[above,draw,xshift=1em,fill=badbee!20]{\scriptsize Überladung?};
      \end{visible}

...
\begin{frame}{Unterschiede der Aufrufkonventionen}
    ...
    \ii<3-> Zusätzliche Parameter liefern Aufrufkontext {
      ...
      \ii Zieladresse für Rückgabe-Objekte im  0. Parameter
      \ei
    }
    \ei
  \end{frame}
...
Nachdem durch die Aufrufkonvention und die Abbildung von IR-Variablen auf Stack-Slots klar ist wie das Datenlayout für einen Call-Frame aussieht, geht es nun darum die IR-Befehl als Maschinenbefehle zu codieren.
Dazu müssen zwei Entscheidungen getroffen werden: Bei der *Befehlsauswahl* wird für einen IR-Befehl ein einzelner oder eine Sequenz von Maschinenbefehlen ausgewählt. Bei der *Registerallokation* entscheiden wir zu welchem Zeitpunkt eine Variable in ihrem Slot lebt und wann sie in einem CPU Register zu finden ist. Haben wir beide Probleme gelößt können wir die Maschinenbefehle, zusammen mit den passenden Registeroperanden als Assembler raus schreiben und sind fertig mit der Übersetzung. Also eigentlich ganz einfach, wären da nicht zwei NP-Vollständige Probleme.
...
Außerdem ist nicht für jeden IR-Befehl 1:1 Abbildung möglich, sondern es wird eine 1:N Abbildung auf Maschinenebefehle benötigt um die geforderte Funktionalität umzusetzen.
...
Dies rührt daher, dass Zugriffe auf den Speicher viel, viel langsamer sind als auf Register[fn::siehe: [[https://www.usenix.org/legacy/events/hotos09/tech/full_papers/baumann/baumann_html/index.htmlttps://www.usenix.org/legacy/events/hotos09/tech/full_papers/baumann/baumann_html/index.html][Your computer is already a distributed system. Why isn't your OS?]]][fn::Zugriffszeiten von [[https://stackoverflow.com/questions/4087280/approximate-cost-to-access-various-caches-and-main-memory#4087315][Stackoverflow]].]
Daher kostet jeder Transfer zwischen Stack-Slot und Register richtig viel Zeit und wir möchten davon, während der Laufzeit, möglichst wenige Slot-Register-Transfers haben.
Aber Registerallokation ist auch ein kompliziertes Problem, wenn man Variablen über Basisblockgrenzen hinweg im Register halten will.
Erwartet der Code eines Basisblocks, zum Beispiel, eine Variable in einen bestimmtem Register, so müssen alle Vorgängerblöcke dafür sorgen, dass die Variable auch tatsächlich in diesem Register geladen ist.
...
#+begin_src latex
  \begin{frame}{Minimale Maschinencodeerzeugung}
    ...
    \ii \structure{Registerallokation}: Spilling {
      \bi
      \ii<handout:5-|5-> \structure{Spilling}: Variable-Wert aus einem Register in den Speicher schreiben.\\[1ex]
      \ii<handout:5-|5-> Variablen immer neuladen
      \ii<handout:5-|5-> Ergebnisse direkt spillen
      \ii<handout:6-|6-> Jede Variable hat einen Speicher-Slot
      \ei
    }
    \ei
      \end{column}\hfill
...
An den Stellen an denen Register in die Operanden verwendet würden, enthält das Makro allerdings Platzhalter (R1, R2), die von der Registerallokation gefüllt werden.
...
Die einfachste Variante einer Registerallokation ist ein Spilling-Register-Allocator.
...
Es gibt keinerlei Probleme, dass es über Kontrollflüsse hinweg zu irgendwelchen Inkonsistenzen kommen kann oder wir vergessen irgendeinen Wert zurückzuschreiben. Schick. Schick aber ineffzient.
...
Allein am Anteil bei dem ~Add~-Beispiel, bei dem 3 von 4 Befehlen nur Transfers waren, dass ein Hauptteil unseres Programms sich damit beschäftigt den Speicher zu beschäftigen.
Viel klüger wäre es nur das aus dem Speicher zu laden, was nicht schon im Register ist und Werte, noch einmal verwendet werden, solange es irgendwie geht in Registern zu halten.
...
Auf den Folien sehen Sie eine Adressberechnung für ein Array, dass einmal naive aus drei Makros zusammegesetzt wurde, und einmal der passende IA-32 Befehl, der in einem Rutsch ~%eax+8*%ebx~ rechnet und dereferenziert.
Die Besonderheiten der Prozessor-Architektur werden also nicht vollständig ausgenützt.
...
Durch dieses größere Wissen wird die Allokation zwar aufwändiger, das Ergebnis aber um Größenordnungen besser.
Um die Prozessorarchitektur auszunützen, haben moderne Übersetzer einen *Peephole-Optimizer*, der immer eine fixe Anzahl an Assemblerinstruktionen (z.B. 3 aufeinanderfolgende Befehle) gegen ein Muster-Datenbank vergleicht und durch effizentere Schnipsel ersetzt.
Um die mikroarchitekturellen Besonderheiten auszunützen kann die *Instruktion-Selection* Assemblerbefehle so umordnen, dass immer noch die gleiche Berechnung durchgeführt wird, aber möglichst viele Instruktionen im Windschatten anderer verdeckt werden.

Im Folgendenen greifen wir uns den Aspekt der Registervergabe heraus und diskutieren einen Registerallokator, der nicht so sehr unter Amnesie leidet und sich, über den Verlauf eines einzelnen Basisblocks hinweg, merkt, welche Variablen bereits in den  Registersatz transferiert wurden.
...
Dieser Allokator mit Gedächtnis sieht den Registersatz als eine Art schnelle Zwischenspeicher (Cache) für die Slots der Variablen im Speicher an.
...
Für letztere werden dann Load-Befehle emitiert um den Cache zu füllen.
...
Dieser Verwaltet nicht nur, wie der Name andeutet, die vollen und freien Register, sondern die Zuordnung von Register zu Variablen.
...
Das eigentliche Makro-Sequenz besteht nur aus der Instruktion ~"add"~, welche in den Assembler ausgegeben bzw.
emittiert wird.
...
In einer funktionalen Hiearchie ist der Registerallokator vollständig abhängig von der Makroexpansion.
Dies bedeutet, dass die Registerallokation niemals von alleine tätig wird, sondern er an neuralgischen Stellen aufgerufen wird um Variablen zu laden bzw.
ihm die Chance gibt sich mit der Maschinencodeerzeugung zu synchronisieren.

Durch drei Hooks, die jeweils vor der Maschinencodeerzeugung für eine Funktion, einen Basisblock oder eine einzelne Instruktion, erhält der Allokator die Möglichkeit seine interne Buchführung entsprechend anzupassen. An diesen Stellen findet kein Informationsfluss vom Allokator zur Makroexpansion statt.

Anders Verhält es sich bei den Funktionen ~alloc_register()~, ~load()~ und ~write()~.
Hier bekommt der Allokator einen konkreten Arbeitsauftrag von der Makroexpansion ein Register frei zu räumen, eine Variable in ein Register zu transferieren, bzw. ein Ergebnis-Register mit einer IR-Variable zu verknüpfen.
Dabei muss die Makroexpansion anfordern können, dass ein konkretes Register alloziert werden soll (~reg~, ~dst_reg~), da manche Maschinenbefehle ihre Operanden in bestimmten Registern erwarten.
...
Fragen Sie danach was der Zustand (Daten) ist und mit nach welchen Regeln diese verarbeitet werden.].
Der Zustand unseres verbesserten Allokators besteht aus 3 Felder pro Prozessor-Register, die während der Maschinencodeerzeugung kontinuierlich geupdatet werden.
...
Die niedrigsten Kosten fallen an, wenn wir ein leeres Register heraus geben. Da kein Wert aus dem Registersatz verdrängt wird, können in der Zukunft auch keine Kosten für ein Neu-Laden des Wertes auftauchen. Leere Register werden also mit der höchsten Priorität vergeben.
...
Dabei weißt uns die Makroexpansion an eine gewisse IR-Variable (~src~) in ein Register zu laden. Hierbei unterscheiden wir zwischen mehreren Fällen, abhängig davon wie der aktuelle Allokatorzustand aussieht:

Findet sich die zu ladende Variable in keinem Register (~load(z)~), so allozieren wir mittels ~alloc_register()~ ein neues Register und emittieren einen Ladebefehl, der die Variable aus ihrem Stack-Slot in das Register transferiert.
...
Zum einen ist eine Registervergabe, die *Basisblockgrenzen* überschreitet viel schwieriger als eine Block-lokale Vergabe. Wieder haben wir hier das Problem, dass nur die Registerbelegungen die in allen Vorgängerknoten konsistent sind, über Blockgrenzen hinweg Gültigkeit haben. Anders ausgedrückt: Eine Variable muss am Ende aller Vorgängerknoten im gleichen Register geladen sein, um bereits vor der ersten Instruktion eines Blocks als geladen zu gelten. Wir machen es uns hier einfach und werfen vor jedem Block einfach den gesamten Allokatorzustand weg.
...
Trotz aller dieser konservativen Annahmen, und das häufig wegwerfen des Allokator-Zustandes hat unser verbesserter Allokator bereits einen erheblichen Einfluss auf die Performance von Programmen.
Um ihnen dies zu verdeutlichen habe ich einen interativen Fibonacci einmal mit der Minimallösung und einmal mit der verbesserten Registervergabe übersetzt.
...
Allerdings sehen Sie auch, dass GCC noch einiges aus einem solchen interativen Fibonacci heraus holt.
...
#+begin_src latex
  ...
  \begin{frame}{Übersicht des restlichen Übersetzungsvorgang}
    ...
    \ii<2-> Aber da ist noch mehr\ldots {
      ...
      \ii<handout:4-|4-> \structure{Gemeinsame Bibliotheken} erlauben es Binärcode zu sharen
      \ei
    }
    \ei
  \end{frame}
...
ELF ist auch eng verbunden mit der *separaten Übersetzung* von Quellcodedateien. Dabei wird der Übersetzer für mehrere Quelldateien separat aufgerufen, mit dem Zwischenschritt über den Assembler, ein Objekt-Datei erstellt. Diese Objektdateien, die bereits Binärdaten enthalten und dem ELF-Format entrsprechen, werden dann vom *Linker* zu ein großen ELF-Datei zusammengebunden. Dabei werden symbolische Referenzen zwischen den Objektdateien aufgelöst, sodass es möglich ist eine Funktion, die in einer anderen Objekt-Datei definiert ist, aufzurufen.
...
Zuerst betrachten wir uns den *Link View* der, wie der Name es andeutet beim Linken von mehreren Objektdateien zu einem ausführbaren Programm verwendet wird. Erzeugt werden die Objektdateien vom Assembler, der aus den *Mnemonics* [fn::Das Wort lässt sich in etwa mit Merkspruch übersetzen und es ist selbst das genaue Gegenteil von dem was es bedeuetet. Ich finde es überhaupt nicht leicht zu merken.] Binärcode erstellt. Neben den Assemblerzeilen als Maschinenbefehle codieren, gibt es auch noch Pseudo-Instruktionen, die die Erzeugung der ELF-Datei steuern. So gibt der Pseudobefehl ~.text~ an, dass ab nun Code kommt, während ~.data~ angibt dass nun Daten folgen. Außerdem beinhaltet Assembler symbolische Namen (~main~, ~foo~), die der Prozessor nicht unterstützt, und die daher in des ELFs landen müssen.
Der Link-View besteht hauptsächlich aus drei wichtigen Elementen: Sektionen, Symbole und Relokationen.

*Sektionen* beinhalten die eigentlichen Binärdaten, die können im Fall der Text-Sektion Maschinenbefehle  sind, und im Falle der Daten-Sektion vorinitialisierte globale Variablen. Sektionen haben also, in der ELF-Datei, einen Anfang, eine Länge und einen Namen. Außerdem hat jede Sektion noch Flags, die angeben ob der Inhalt der Sektion einmal in den Prozess geladen werden soll (A), dort ausführbar ist (X) oder geschrieben werden soll (W). Da der Assembler noch nicht alle symbolischen Referenzen auflösen kann (z.B. zu Funktionen aus anderen Objekt-Dateien), lässt er an entsprechender Stelle Platzhalter, die vom Linker aufgefüllt werden. Sie sollten einmal her gehen und eine Objektdatei mittels ~readelf -a~ (Metadaten) ~objdump -D~ (Disassembly) dekodieren um ein Gefühl für den Inhalt dieser Sektionen zu bekommen.

Neben der eigentlichen Nutzlast, die in Sektionen gespeichert, gibt es noch die Symboltabelle[fn::Technisch gesehen ist die Symboltabelle auch eine Sektion, die allerdings nicht in den Prozess geladen wird.
...
Wenn diese in dieser ELF-Datei definiert sind, so referenziert ein Symbol ein bestimmtes Byte in einer der Sektionen.
So zeugt das Symbol ~main~ in die Text-Sektion (~T~) auf das Byte mit dem Wert ~0x55~, was dem IA-32 Befehl ~push %ebp~ entspricht und den Beginn der Funktion markiert.
Aber ein Symbol kann auch noch undefiniert (~U~) sein, wie das bei ~fib~ der Fall ist.
Eine solche Situation zeigt an, dass das Symbol in einer anderen Objekt-Datei definiert sein muss.
Die Auflösung der undefinierten Symbole übernimmt der Linker.
Um sich die Symbole einer ELF-Datei zu betrachten, rufen Sie einmal das Tool ~nm~ auf einer Objektdatei auf.
...
Da manche Platzhalter eine relative (Funktionsaufrufe auf IA-32 sind Funktionsrelativ) und manche eine absolute Adressierung (Referenzerzeugung einer Variable) verlangen, hat jede Relokation noch einen Typ, der angibt wie die Adresse des Symbols in die Binärdaten hinein gepatcht werden sollen.


Relokationen spielen sowohl beim Linken als auch beim Laden eine Rolle. Wenn ein Programm es erlauben soll, an unterschiedliche Stellen in den RAM geladen zu werden, so müssen alle Codestellen, die eine absolute Adressieren verlangen, entsprechend angepasst werden. Insbesondere bei geteilten Bibliotheken spielen diese Run-Time Relocations eine große Rolle.

Was geschiet nun also beim Linken mehrerer Objektdateien?
...
Nachdem die Sektionen gesammelt und verbunden sind, wird auch die Symboltabelle vereinigt. Dabei werden undefinierte Symbole, soweit möglich, durch definierte Symbole aus anderen Objekt-Dateien (oder aus Bibliotheken) augelöst. Soll am Ende eine ausführbare Datei entstehen, darf kein Symbol unaufgelöst bleiben. Ist dies doch der Fall kommt es zum Fehler zu einem Linker Fehler.

Zusätzlich werden beim Linken alle Relokationen aufgelöst, die nun auf nun definierte Symbole zeigen.
Dazu modifiziert der Linker die Platzhalter in den Sektionen entsprechend des Relokations-Typs und des Symbolwertes.
...
Daher wird ihnen ihnen ~nm /bin/bash~ nur ~nm:
...
Auf der Seite des ELFs besteht ein Segment aus einem ~Offset~, der relativ zum Beginn der ELF-Datei berechnet wird, der Länge in der Datei (~FileSiz~).
...
Beim Segment dass die Daten-Sektion enthält gilt ~FileSiz < MemSize~.

Der Loader iteriert über die Segmente und fordert entsprechende Bereiche im Adressraum an.
In diese Bereich werden dann die Daten aus der ELF Datei, entsprechend der Segmente, kopiert und beim Betriebssystem entsprechend als ausführbar oder schreibbar markiert. Nach dem Ladeprozess springt der Loader an den Einstiegspunkt, der ebenfalls in den ELF Metadaten gespeichert wird.
...
Hier sehen wir, dass 5 Shared Libraries bei Start einer Bash-Shell geladen werden.
...
#+begin_src latex
  \begin{frame}{Zusammenfassung}
    ...
    \ii \structure{Call-Frame} enthält Argumente, Rücksprungadresse und Variablen-Slots{
      \bi

...
#+begin_src latex
  \begin{frame}[fragile]{Zusammengesetzte Typen: Records}
    \begin{columns}
      \begin{column}{0.49\textwidth}
        \begin{code}[tag=C]
          \begin{C}
...
- Was tun, wenn meine Sprache einen Typ nicht anbietet? Minimale Wunsch ist: Record+Struct oder
...
#+begin_src latex
  ...
  \begin{frame}[fragile]{Polymorphismus macht Code flexibel}
    ...
    \ii Typsysteme die immer strikt anhand der Gleicheit prüfen sind \ALERT{inflexibel} {
      \bi
    ...
    \ii Polymorphismus fängt diese Ähnlichkeit zwischen Typen ein{
      \bi
      \ii Ein Typ kann viele Gestalten haben (Griechisch: poly=many, morph=form)
      \ii Polymorpher Code darf sich nur auf die Gemeinsamkeit verlassen
      \ii Subtype Polymorphismus, Parametrischer Polymorphismus, Ad-Hoc Poly....
      \ei
    }
    \ei
  \end{frame}
  ...
  \begin{frame}[fragile]{Typkompatibilität}
    ...
    \ii \enquote{Sicher} hat dabei viele unterschiedliche Interpretationen{
      \bi
      \ii Speicher: Ich kann der Sprache keine Katze als Segelschiff unterschieben.
    ...
    \ii<2> \alert{Keine} Typsystem kann alles abfangen und komplette Sicherheit bieten{
    \vspace{-2ex}
  \begin{columns}[t]
    \begin{column}{0.49\textwidth}
  ...
  \begin{frame}{Subtype Polymorphismus}
  \end{frame}

  \begin{frame}{Parametrische Typen}
  \end{frame}

...
#+begin_src latex
    ...
    \begin{frame}{Parametrisch: Typkonstruktoren werden selbst Typen}
      \begin{btBlock}[type=alert]{Generische Typen als First-Class Citizens}
      ...
      \ii Der generische Typ ist kompatibel mit all seinen \textbf{konkreten Instanzen}{
        \bi
        \ii Im Gegensatz zu Vererbung gibt es (potentiell) unendlich viele Instanzen.
        \ii \typeinline{Pair(A)} ist kompatibel mit \typeinline{Pair(int)}, \typeinline{Pair(bool)}, \ldots
    ...
    \begin{frame}{Instanztypen: Covarianz, Contravarianz, und Invarianz}
    ...
    \ii<2-> \structure{Kontravarianz}: Kompatibilität läuft entgegen der Spezialisierung{
      \bi
    ...
    \ii<3-> \structure{Invarianz}: Instanztypen sind inkompatibel zueinander{
      \bi
    ...
    \ei
    \OrangeBox[putat={(4.5cm, 5cm)},visible on=<1>,align=left]{\footnotesize

...
#+begin_src latex
  %%ONLY%%
  \begin{frame}[fragile]{Zusammengesetzte Typen: Records und Unions}
    \begin{columns}
      \begin{column}{0.49\textwidth}
        \begin{code}[tag=C]
          \begin{C}
    ...
    \bi
    \ii Record sind das Produkt von heterogenen (unterschiedlichen) Typen {
      \bi
      \ii Die einzelnen Komponenten eines Records heissen Felder
      \ii Jedes Feld hat einen eigenen Typ und (normalerweise) einen Namen.
    ...
    \medskip
    \ii<2-> Bei Unions (Variant Records) ist maximal ein Feld gültig {
      \bi
  ...
  \begin{frame}{Zusammengesetzte Typen: Listen und Sets}
  \end{frame}

  \begin{frame}{Zusammengesetzte Typen: Arrays}
  \end{frame}
...
- Kompatibilität von Typen (Neben der Äquivalenz gibt es noch die Kompatibilität)
- Parametrische Typen
- Co/Contravarianz
- Type Traits
